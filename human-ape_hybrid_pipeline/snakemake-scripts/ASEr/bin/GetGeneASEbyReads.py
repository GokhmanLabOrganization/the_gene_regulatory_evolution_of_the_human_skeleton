...
def get_snps(snpfile):
    # returns a dict of dicts: snps[chrom][position] = [ref, alt]"""
...
def get_gene_coords(gff_file, id_name, feature_type='exon'):
    # Returns a dict: gene_coords[gene_id] = (chrom, set((left, right), ...))
...
def get_ase_by_coords(chrom, coords, samfile, snp_dict):
    # returns a Counter of read counts per phase
    left_most = 1e99
    right_most = 0
...
def get_lib_size(reads):
    # command to get total number of alignments/reads reported in the BAM index (requires BAM to be indexed)
...
def compute_gene_length(coord_set):
    """
    coord_set: a set of (left, right) positions (GFF-style, 1-based inclusive).
    Returns union length of all exons in base pairs.
    """
    if not coord_set:
        return 0

    # Turn into a sorted list
    intervals = sorted(coord_set, key=lambda x: x[0])

    merged = []
    cur_start, cur_end = intervals[0]

    for start, end in intervals[1:]:
        # Overlap or adjacency (inclusive coordinates)
        if start <= cur_end + 1:
    ...
    # Sum lengths; GFF is inclusive → length = end - start + 1
    length_bp = sum(e - s + 1 for s, e in merged)
    return length_bp


def counts_to_tpm(counts_dict, gene_lengths_dict):
    # Convert counts to TPM
    # RPK
    rpk = {}
    for g, cnt in counts_dict.items():
        length_bp = gene_lengths_dict.get(g, 0)
        if cnt > 0 and length_bp > 0:
    ...
    tpm = {}
    for g, val in rpk.items():
        if total_rpk > 0 and val > 0:
            tpm[g] = (val / total_rpk) * 1e6
        else:
            tpm[g] = 0.0
    return tpm
...
if __name__ == "__main__":
    ...
    snp_dict = get_snps(args.snp_file) # returns a dict of dicts: snps[chrom][position] = [ref, alt]"""
    gene_coords = get_gene_coords(args.gff_file, args.id_name) # Returns a dict: gene_coords[gene_id] = (chrom, set((left, right), ...))

    # Compute gene lengths from exon coordinates
    gene_lengths = {}
    for gene, (chrom, coord_set) in gene_coords.items():
        gene_lengths[gene] = compute_gene_length(coord_set)
    ...
    if False and args.max_jobs != 1:
        ...
        # wants to put it back, it should be easy...
        ...
        for gene in prog(gene_coords):
            if gene_coords[gene][0] not in snp_dict.keys(): # no snps on this chromosome
                continue
            ase_vals[gene] = get_ase_by_coords(
                gene_coords[gene][0], # chrom
                gene_coords[gene][1], # coords
                reads, # samfile
                snp_dict # snp_dict
            )
        if 'finish' in dir(prog):
    ...
    # Build per-gene counts for TPM
    ref_counts = {}
    alt_counts = {}
    total_counts = {}

    for gene, vals in ase_vals.items():
        # vals is a dict-like: [-1]=ref, [1]=alt, [None]=no_ase, [0]=ambig
        ref = vals.get(-1, 0)
        alt = vals.get(1, 0)
        no_ase = vals.get(None, 0)
        ambig = vals.get(0, 0)

        ref_counts[gene] = ref
        alt_counts[gene] = alt
        total_counts[gene] = ref + alt + no_ase + ambig


    # Three separate TPM values per gene, one for ref, alt, and total
    # each set of TPMs sums to 1e6 across genes
    ref_tpm   = counts_to_tpm(ref_counts,   gene_lengths)
    alt_tpm   = counts_to_tpm(alt_counts,   gene_lengths)
    total_tpm = counts_to_tpm(total_counts, gene_lengths)
    

    # headers for output file
    print("# Library size: " + str(lib_size), file=args.outfile, end='\n')
    print("# Run date: " + str(datetime.datetime.now()), file=args.outfile, end='\n')
    print("# GFF used: " + str(args.gff_file), file=args.outfile, end='\n')
    print("# fasta used: " + str(args.fasta), file=args.outfile, end='\n')
    print("# SNPs used: " + str(args.snp_file), file=args.outfile, end='\n')
    print("# SNPs wasp used" + str(args.snp_wasp), file=args.outfile, end='\n')
    # Column headers
    print('gene', 'chrom',
          'ref_counts', 'alt_counts', 'no_ase_counts', 'ambig_ase_counts',
          'ase_value',
          'ref_tpm', 'alt_tpm', 'total_tpm',
          file=args.outfile, sep='\t', end='\n')
    
    # Output per-gene ASE values
    for gene in sorted(ase_vals):
        ...
        print(
            gene,
            gene_coords[gene][0],
            ase_vals[gene][-1],      # ref_counts
            ase_vals[gene][1],       # alt_counts
            ase_vals[gene][None],    # no_ase_counts
            ase_vals[gene][0],       # ambig_ase_counts
            ase_val,
            ref_tpm.get(gene, 0.0),
            alt_tpm.get(gene, 0.0),
            total_tpm.get(gene, 0.0),
            file=args.outfile,
            sep='\t',
            end='\n'
        )
