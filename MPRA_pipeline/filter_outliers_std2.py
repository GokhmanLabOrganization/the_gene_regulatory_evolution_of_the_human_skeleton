...
cells = sys.argv[3]
# output paths
filtered_oligo_bc_path = f'./{cells}/{library}{adaptor}/output/filter/filtered_oligo_bc.csv'
filtered_out_oligo_bc_file_path = f'./{cells}/{library}{adaptor}/output/filter/filtered_out_oligo_bc.txt'
...
def std_n_outliers(rna, dna, oligo_bc, n):
    non_zero_indices = sorted(set(i for i, rna_val in enumerate(rna) if rna_val > 0) |
                 set(i for i, dna_val in enumerate(dna) if dna_val > 0))
    filtered_oligo_bc=[]
    if non_zero_indices:
        zero_indices = [i for i,v in enumerate(rna) if i not in non_zero_indices]
        #zero_rna_indices = sorted(set(i for i, rna_val in enumerate(rna) if rna_val == 0))
        non_zero_rna = [rna[i] for i in non_zero_indices]
        non_zero_dna = [dna[i] for i in non_zero_indices]
        mean = np.mean(non_zero_rna)
        std = np.std(non_zero_rna)
        lower_bound = mean - n * std
        upper_bound = mean + n * std
        valid_indices = [i for i, v in enumerate(rna) if lower_bound < v < upper_bound]
        valid_indices = list(set(valid_indices + zero_indices))
        filtered_indices = [i for i,v in enumerate(rna) if i not in valid_indices]
        rna = [rna[i] if i not in filtered_indices else 0.0 for i in range(len(rna))]
        dna = [dna[i] if i not in filtered_indices else 0.0 for i in range(len(dna))]
        filtered_oligo_bc = [oligo_bc[i] for i in filtered_indices]       
    return rna, dna, oligo_bc,filtered_oligo_bc
...
def create_ratio_df():
    ...
    for i, rep in enumerate([rep1, rep2, rep3]):
        ...
        for index, row in rep_list.iterrows():
            rep_list.at[index, 'RNA_filtered_std2'], rep_list.at[index, 'DNA_filtered_std2'], rep_list.at[index, 'oligo_bc_filtered_std2'],filtered_oligo_bc = std_n_outliers(
                row['RNA_no_filter'], row['DNA_no_filter'], row['oligo_bc_no_filter'], 2)
            filtered_oligo_bc_list.extend(filtered_oligo_bc)
    ...
    UMI_comb_filter = exploded_df_filter.copy()
    ...
    UMI_comb_no_filter = exploded_df_no_filter.copy()
