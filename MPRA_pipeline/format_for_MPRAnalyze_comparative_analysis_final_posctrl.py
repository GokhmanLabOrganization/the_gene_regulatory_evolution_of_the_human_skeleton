...
pickle_path = f'/home/labs/davidgo/Collaboration/humanMPRA/{cells}/{library}{adaptor}/output/mpranalyze_comparative/{project}/{cells}_{library}{adaptor}_MPRAnalyze_dict_nocontrols_improved.pickle'
...
def main():    
    ...
    def turn_into_locus(f):
        ...
        to_fuse = oligo_split[:5] + oligo_split[6:-1]
        locus = "_".join(to_fuse)
        return locus


    def turn_into_locus_on_fasta(f): # have to use different function becuase fasta does not contain barcode
        oligo_split = f.split("_")
        to_fuse = oligo_split[:5] + oligo_split[6:]
        locus = "_".join(to_fuse)
        return locus
        
    def get_allele_source(f):
        oligo_split = f.split("_")
        allele = oligo_split[5]  # either "Ancestral" or "Derived"
        return allele
    ...
    data_df=data_df[data_df.index.to_series().apply(lambda x: (bool(re.findall("PosCtrl_diff",x))))]
    ...
    no_dups_loci=data_df['locus'].unique()  # there will be dups because of ancestral or derived for each
...
def cell_types_separate(cell_dfs):    
    ...
    def make_annot_table(df):
        ...
        annot_table.to_csv(f'/home/labs/davidgo/Collaboration/humanMPRA/{cells}/{library}{adaptor}/output/mpranalyze_comparative/{project}/{cells}_{library}{adaptor}_col_annot_comparative_improved.txt', sep='\t', header=True)
    ...
    final_df_RNA.to_csv(f'/home/labs/davidgo/Collaboration/humanMPRA/{cells}/{library}{adaptor}/output/mpranalyze_comparative/{project}/{cells}_{library}{adaptor}_RNA_MPRAnalyze_comparative_improved.txt', sep='\t', header=True, index_label='locus')
    final_df_DNA.to_csv(f'/home/labs/davidgo/Collaboration/humanMPRA/{cells}/{library}{adaptor}/output/mpranalyze_comparative/{project}/{cells}_{library}{adaptor}_DNA_MPRAnalyze_comparative_improved.txt', sep='\t', header=True, index_label='locus')
