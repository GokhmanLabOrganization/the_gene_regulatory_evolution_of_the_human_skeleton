...
csplit -f ./$cells/$library$adaptor/output/mpranalyze_comparative/rna_chunks_filter_sorted/rna -s -n 3 ./$cells/$library$adaptor/output/mpranalyze_comparative/${cells}_${library}${adaptor}_RNA_MPRAnalyze_comparative_filter_adjusted_fdr_sorted.txt $(cat ./$cells/$library$adaptor/output/mpranalyze_comparative/split_positions.txt)
csplit -f ./$cells/$library$adaptor/output/mpranalyze_comparative/dna_chunks_filter_sorted/dna -s -n 3 ./$cells/$library$adaptor/output/mpranalyze_comparative/${cells}_${library}${adaptor}_DNA_MPRAnalyze_comparative_filter_adjusted_fdr_sorted.txt $(cat ./$cells/$library$adaptor/output/mpranalyze_comparative/split_positions.txt)
