...
def loop_through_bams():
    ...
    UMI_diffs_df = pd.DataFrame({'oligo_bc': UMI_diffs['All']}) #temp for QC pipleine, can be removed!!!
    UMI_diffs_df.to_csv(f'{project_dir}/UMI_complexity_for_QC_pipeline.txt', sep='\t', header=True, index_label='oligo_bc') #temp for QC pipleine, can be removed!!!
    return #temp, can be removed!!!
...
def main():
    if not os.path.exists(data_df_path):
        ...
        barcode_coverage, data_df = loop_through_bams()#temp for QC pipleine, can be removed!!!
